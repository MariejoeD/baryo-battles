...
@onready var wood_label =$UI/Bodega/viewInformation/InformationPanel/HBoxContainer/woodAmount
@onready var stone_label =$UI/Bodega/viewInformation/InformationPanel/HBoxContainer/stoneAmount
...
func _process(delta: float) -> void:
	# Update wood and stone amounts in real-time
	wood_label.text = "Wood: %d" % Global.wood_qty
	stone_label.text = "Stone: %d" % Global.stone_qty
...
func find_nearest_sibilyan() -> Node:
	for kubo in Global.all_kubos:
		if kubo.stored_sibilyans.size() > 0:
			...
			get_tree().get_root().get_node("Root/Base/Entities").add_child(sib)
